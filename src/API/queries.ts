...
export const getTrendingMovies = async () => {
  try {
    const result = await axiosClient.get<GetResult<RawMovie>>(
      '/trending/movie/week'
    );
...
export const getPopularTitles = async (mediaType: MediaType) => {
  ...
  try {
    const result = await axiosClient.get<GetResult<RawMovie | RawTVShow>>(
      `/discover/${mediaType}?primary_release_date.gte=${startDate}&primary_release_date.lte=${endDate}`
    );
...
export const getTrailer = async ({ id, mediaType }: GetTrailerParams) => {
  try {
    const result = await axiosClient.get<TrailerResult>(
      `/${mediaType}/${id}/videos`
    );
...
export const getFullTitle = async ({ id, mediaType }: GetFullTitleParams) => {
  try {
    const result = await axiosClient.get<FullMovie | FullTVShow>(
      `/${mediaType}/${id}?append_to_response=keywords,credits,similar`
    );
    return result.data;
  } catch (err) {
    console.error(err);
...
export const getReviews = async ({ id, mediaType, page }: GetReviewsParams) => {
  try {
    const result = await axiosClient.get<GetResult<Review>>(
      `/${mediaType}/${id}/reviews?page=${page}`
    );
    return result.data;
  } catch (err) {
    console.error(err);
